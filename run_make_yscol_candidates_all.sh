...
MACRO="${PROJECT_DIR}/make_yscol_candidate_tree.C"
...
STEP_DIR="${CAMPAIGN_DIR}/04a_candidate_trees_y"
...
while IFS=$'\t' read -r tag optics_id angle foils nruns runs input_root; do
  ...
  output_root="${ROOTDIR}/YscolCandidates_${tag}.root"
  output_tsv="${TSVDIR}/YscolCandidates_${tag}_summary.tsv"
  logfile="${LOGDIR}/make_yscol_candidates_${tag}.log"

  expression="make_yscol_candidate_tree.C(${optics_id},\"${tag}\",\"${CAMPAIGN}\",\"${input_root}\")"
