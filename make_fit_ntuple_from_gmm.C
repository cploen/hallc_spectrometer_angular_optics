...
void make_fit_ntuple_from_gmm(
  ...
  const vector<TString> maskDirs = coreFile.Length() > 0
      ? vector<TString>{coreFile} : vector<TString>{yBaseDir, xBaseDir};
  for (const auto& maskDir : maskDirs) {
    hallc::Spectrometer maskSpec;
    if (!hallc::loadSpectrometer(maskDir,maskSpec) || maskSpec.name!=spec.name) {
      std::cerr << "ERROR: Event mask and output campaign spectrometers disagree." << std::endl;
      return;
    }
  }
