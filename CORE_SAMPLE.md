...
to be evaluated separately. `build.json` records metadata and macro hashes.
...
The adapter is currently HMS-only because the existing macro uses HMS branches,
9-by-9 target geometry, and the HMS sieve distance. The selector itself supports
other hole counts. A SHMS target adapter is a separate change.
