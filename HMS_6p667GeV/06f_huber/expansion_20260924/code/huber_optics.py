"""Smooth Huber regression on a fixed optical basis, without population weights.
...
def loss_gradient(beta, q, target, delta):
    residual = q @ beta - target
    norm = np.hypot(1., residual / delta)
    # Algebraically delta**2*(norm-1), avoiding cancellation near zero.
    loss = np.mean(residual**2 / (norm + 1.))
    gradient = q.T @ (residual / norm) / len(target)
    return float(loss), gradient


class FixedBasis:
    def __init__(self, x, rcond=1e-12):
    ...
    def fit(self, target, sigma=None, delta=1.5, max_iter=300):
        ...
        start = self.q.T @ t/self.n
        result = minimize(loss_gradient, start, args=(self.q, t, delta), jac=True,
        ...
        return coef, dict(loss='pseudo_huber', delta=delta, sigma=float(sigma),
                          ...
                          effective_n=float(weights.sum()**2/(weights @ weights)))
