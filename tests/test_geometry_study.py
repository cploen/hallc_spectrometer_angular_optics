...
void fixture(const char* path, bool shms) {
 ...
 double reactx=0,reacty=0,reactz=0,xptar=0,sumnpe=5,etracknorm=1;
#define B(v) tree.Branch(#v,&v);
 ...
 B(reactx) B(reacty) B(reactz) B(xptar) B(sumnpe) B(etracknorm)
 ...
 for(double foil:{-8.,0.,8.}) for(int i=-100;i<=100;++i) {
   zfoil=reactz=foil;reacty=.0005*i;
...
''')
            ...
            for arm in ('HMS','SHMS'):
                ...
                for foil in (0,8):
                    run=subprocess.run([sys.executable,str(PROJECT/'geometry_study.py'),str(campaign),'--foil',str(foil)],
                        env={**os.environ,'OPTICS_METADATA':str(metadata)},capture_output=True,text=True)
                    self.assertEqual(run.returncode,0,run.stdout+run.stderr)
                    with (campaign/f'results/foil_{foil}cm/rg01/slopes.tsv').open() as stream:
                        rows=list(csv.DictReader(stream,delimiter='\t'))
                    rows=[r for r in rows if r['subset']=='core' and r['ndel']=='-1']
                    self.assertEqual(len(rows),2)
                    for row in rows:
                        self.assertEqual(int(row['n']),201) # noncores and other foils excluded
