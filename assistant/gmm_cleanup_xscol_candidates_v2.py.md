# Source: gmm_cleanup_xscol_candidates_v2.py

```python
#!/usr/bin/env python3
"""
GMM cleanup for XFP/XPFP xscol candidate events already extracted into TXCand.

This script does NOT reopen or reapply TCutG files.  The candidate tree is assumed
already selected by:
  baseline cuts + ytar/delta foil cut + XFP/XPFP xscol band cut.
...
Example:
  python3 gmm_cleanup_xscol_candidates_v2.py \
    --run 69840 --foil 0 --ndel 3 \
    --input-root hist/XscolCandidates_69840_-1.root \
    --tree TXCand \
    --outdir hist/gmm_cleanup \
...
def read_candidate_tree(args) -> List[EventRecord]:
    ...
    for i in range(nentries):
        ...
        xscol = int(val(tree, br_xscol))

        if foil != args.foil or ndel != args.ndel:
            continue
        if args.xscol is not None and xscol != args.xscol:
            continue
...
def fit_one_xscol(records: List[EventRecord], args) -> Dict[str, float | int | str]:
    n = len(records)
    summary: Dict[str, float | int | str] = {
        "xscol": records[0].xscol if records else -999,
        "n_before": n,
        "n_after": 0,
...
def process_records(records: List[EventRecord], args) -> List[Dict[str, float | int | str]]:
    summaries = []
    for xscol in sorted({r.xscol for r in records}):
        sub = [r for r in records if r.xscol == xscol]
        summaries.append(fit_one_xscol(sub, args))
    return summaries
...
def write_root(records: List[EventRecord], summaries: List[Dict[str, float | int | str]], root_path: Path, args) -> None:
    ...
    t = ROOT.TTree("GMMClean", "GMM event mask for xscol candidates in one rungroup/foil/delta slice")
    ...
    ts = ROOT.TTree("GMMSummary", "GMM cleanup summary by xscol")
    s_xscol = array.array("i", [0])
...
def write_csv(records: List[EventRecord], summaries: List[Dict[str, float | int | str]], csv_path: Path, summary_path: Path, args) -> None:
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "rungroup", "entry", "run", "foil", "ndel", "xscol",
        ...
        for r in records:
            w.writerow([
                args.rungroup, r.entry, r.run, r.foil, r.ndel, r.xscol,
    ...
    with open(summary_path, "w", newline="") as f:
        ...
        w.writerow([
            ...
            "xscol", "n_before", "n_after", "keep_frac_actual",
        ...
        for s in summaries:
            w.writerow([
                ...
                s["xscol"], s["n_before"], s["n_after"], s["keep_frac_actual"],
                s["action"], s["n_components"], s["bic"], s["score_threshold"],
            ])


def draw_xsieve_guides(ax, args) -> None:
    ...
    for nxs in range(9):
        ...
        ax.text(
            ...
            ha="right", va="center", fontsize=7, color="red", clip_on=False,
        )
...
def style_sieve_axis(ax, args) -> None:
    ...
    ax.minorticks_on()
    draw_xsieve_guides(ax, args)


def make_pdf(records: List[EventRecord], summaries: List[Dict[str, float | int | str]], pdf_path: Path, args) -> None:
    with PdfPages(pdf_path) as pdf:
        # Page 1: all color-coded xscol projections, before and after.
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        for xscol in sorted({r.xscol for r in records}):
            sub = [r for r in records if r.xscol == xscol]
            axes[0].scatter([r.ysieve for r in sub], [r.xsieve for r in sub], s=7, label=f"x{xscol}")
            kept = [r for r in sub if r.gmm_keep]
            if kept:
                axes[1].scatter([r.ysieve for r in kept], [r.xsieve for r in kept], s=7, label=f"x{xscol}")
        axes[0].set_title("All xscol projections BEFORE GMM")
        axes[1].set_title("All xscol projections AFTER GMM")
        for ax in axes:
            style_sieve_axis(ax, args)
            ax.legend(fontsize=7, loc="best")
        fig.suptitle(f"{args.rungroup} — foil {args.foil}, {delta_label(records)}")
        fig.tight_layout()
        pdf.savefig(fig)
        plt.close(fig)

        # Per-xscol diagnostic pages.
        for s in summaries:
            xscol = int(s["xscol"])
            sub = [r for r in records if r.xscol == xscol]
            ...
            xpfp = np.array([r.xpfp for r in sub])
            xfp = np.array([r.xfp for r in sub])

            ax1.scatter(xpfp, xfp, s=8)
            ax1.set_title(f"XFP/XPFP candidate reference: xscol {xscol}")
            ax1.set_xlabel("xpfp")
            ax1.set_ylabel("xfp")
            ...
            fig.suptitle("xscol candidate → GMM sieve-space cleanup", fontsize=14)
...
def output_paths(
    ...
    selection = "allX" if args.xscol is None else f"xscol{args.xscol}"
    tag = (
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="GMM cleanup for TXCand xscol candidates in xsieve/ysieve space."
    )

    p.add_argument(
        "--rungroup",
        required=True,
        help="Full rungroup tag used for campaign I/O and labels.",
    )
    p.add_argument(
        "--campaign",
        type=Path,
        required=True,
        help=(
            "Campaign path relative to the current project-root directory, "
            "e.g. HMS_6p117GeV."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Optional: process only one xscol. Default processes all present xscols.",
    )
    p.add_argument(
        "--input-root",
        type=Path,
        default=None,
        help="Override candidate ROOT input.",
    )
    p.add_argument("--tree", default="TXCand")
    p.add_argument(
        "--outdir",
        type=Path,
        default=None,
        help="Override 05b_gmm_cleanup_x output directory.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=30,
        help="Remove candidate columns with fewer than this many events.",
    )
    p.add_argument(
        "--max-components",
        type=int,
    ...
    p.add_argument(
        ...
        type=float,
        default=0.95,
        help="Fraction retained after GMM log-density cut.",
    )
    p.add_argument(
        "--min-points-per-component",
        type=int,
        default=5,
        help="Guard against tiny nuisance components.",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=1.524,
        help="Legacy plotting arg; X guides are fixed xsieve rows.",
    )
    p.add_argument(
        "--guide-origin",
        type=float,
    ...
    p.add_argument(
        ...
        type=int,
        default=4,
        help="Row index located at guide-origin.",
    )
    ...
    if args.input_root is None:
        args.input_root = (
            ...
            / "root"
            / f"XscolCandidates_{args.rungroup}.root"
        )

    if args.outdir is None:
        args.outdir = args.campaign_dir / "05b_gmm_cleanup_x"
    ...
    if not records:
        raise RuntimeError(
            ...
            f"foil={args.foil} ndel={args.ndel} xscol={args.xscol}"
        )

    summaries = process_records(records, args)

    root_path, csv_path, summary_path, pdf_path = output_paths(
        args,
        records,
    )
    ...
    print(f"  xscols      : {sorted({r.xscol for r in records})}")

    for s in summaries:
        print(
            f"  x{s['xscol']}: "
            f"action={s['action']} "
            f"N={s['n_before']} -> {s['n_after']} "
