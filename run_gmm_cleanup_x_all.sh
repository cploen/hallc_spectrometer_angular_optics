...
SCRIPT="${PROJECT_DIR}/gmm_cleanup_xscol_candidates_v2.py"
CANDIDATE_DIR="${CAMPAIGN_DIR}/04b_candidate_trees_x"
OUTPUT_DIR="${CAMPAIGN_DIR}/05b_gmm_cleanup_x"
...
MAX_COMPONENTS="${MAX_COMPONENTS:-$HALLC_NY}"
...
if ! awk -v x="$KEEP_FRAC" 'BEGIN { exit !(x > 0.0 && x <= 1.0) }'; then
  echo "ERROR: KEEP_FRAC must be in (0,1]"
  exit 1
fi
...
echo "Rungroups     : $RUNGROUPS_TSV"
echo "Min events    : $MIN_EVENTS"
echo "Max components: $MAX_COMPONENTS"
...
while IFS=$'\t' read -r tag optics_id angle foils nruns runs input_root; do
  ...
  candidate_root="${CANDIDATE_DIR}/root/XscolCandidates_${tag}.root"
  summary_tsv="${CANDIDATE_DIR}/tsv/XscolCandidates_${tag}_summary.tsv"
