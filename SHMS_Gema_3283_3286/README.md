# Gema runs 3283–3286: initial SHMS campaign

The `SHMS_` campaign name alone selects the spectrometer. These are initial inputs for individual-run checks, not a validated calibration or approved set of cuts.
...
- User instruction: assume Gema's foil coordinates **−10, 0, +10 cm for all four runs**. Do not require reconfirmation before initial processing. This target is independent of arm and is also available for HMS.
...
```sh
bash diagnostics/run_gema_replay_probe.sh SHMS_Gema_3283_3286 > gema_replay_probe.txt 2>&1
```
